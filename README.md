<h1 align="center">👋 Hello, I'm <span style="color:#D72638;">Sri Poorvaja Ilipilla</span></h1>

<p align="center">
  <img src="https://img.shields.io/badge/Business Analyst-%230073b9?style=for-the-badge&logo=tableau&logoColor=white"/>
  <img src="https://img.shields.io/badge/Data Analyst-%234B8BBE?style=for-the-badge&logo=python&logoColor=white"/>
  <img src="https://img.shields.io/badge/Snowflake-%2300C5FF?style=for-the-badge&logo=snowflake&logoColor=white"/>
  <img src="https://img.shields.io/badge/SQL-%2300C853?style=for-the-badge&logo=mysql&logoColor=white"/>
</p>

<p align="center">
  <b>Transforming Data into Strategic Insights | Dashboard Developer | Cloud Data Engineer</b>
</p>

---

## 🔍 About Me

🎓 Master’s in Information Systems & Technologies, University of North Texas (2024)  
💼 4+ years in Business & Data Analysis across healthcare, banking, and insurance  
📈 Expert in BI, SQL, Python, Tableau, and Snowflake  
🌎 Passionate about solving real-world business problems using data

---

## 🌐 Portfolio Website

📍 [**View My Work & Dashboards Here**](https://preview--sripoorvaja-insights.lovable.app/)

---

## 🧠 Skills

| Data Analytics | Data Engineering | BI Tools         | Cloud Platforms |
|----------------|------------------|------------------|-----------------|
| SQL, Python     | Informatica, Spark | Tableau, Power BI | AWS, Databricks |
| ETL Pipelines   | SSIS, Snowflake   | Qlik Sense        | Snowflake, Redshift |

---

## 💼 Featured Projects

### 📊 TCRD Risk Dashboard Migration
<img src="https://i.postimg.cc/j2DRFM4G/Chat-GPT-Image-May-18-2025-04-12-24-PM.png" alt="TCRD Risk Dashboard" width="600"/>
<img src="https://i.postimg.cc/59RDfBJN/Chat-GPT-Image-May-18-2025-04-19-18-PM.png" alt="TCRD Risk Dashboard" width="600"/>
- Automated risk scoring using Python & Tableau  
- ⏱ Reduced risk assessment time by **40%**

---

### 🏥 Medicaid Business Intelligence Data Mart
<img src="https://your-dashboard-image-url-2.png" alt="Medicaid BIDM" width="600"/>
- Built healthcare reporting data mart with ETL automation  
- ✅ Increased accuracy by **45%**, faster queries

---

### 🔍 Fraud Detection Model
<img src="https://your-dashboard-image-url-3.png" alt="Fraud Detection Model" width="600"/>
- Python-based machine learning model  
- 🎯 Boosted fraud identification accuracy by **28%**

---

### 📈 HEDIS Real-Time Dashboard
<img src="https://your-dashboard-image-url-4.png" alt="HEDIS Dashboard" width="600"/>
- Enabled live tracking of regulatory KPIs  
- 📋 Improved compliance by **20%**

---

### 🔢 Financial Data Reconciliation
- Used Informatica + Python for automation  
- 🧾 Achieved **98%** report accuracy

---

## 📬 Let's Connect

[![Email](https://img.shields.io/badge/Email-D72638?style=flat-square&logo=gmail&logoColor=white)](mailto:ilipilla.sripoorvaja@gmail.com)

---

> *“Empowering businesses through smarter data decisions.”*

